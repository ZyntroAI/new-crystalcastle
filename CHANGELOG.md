...
- **PR #226** — docs: rewrite `README.md` to match the real repo tree.
  ...
  `README.md` on `main` was not documentation but a pasted Thai chat message,
  produced by an earlier chat-based edit that was never verified against the
  repository. Checking the draft against the tree showed claims that never held:
  `frontend/package.json` and `frontend/.env.example` do not exist, there is no
  root `requirements.txt` (it is `backend/requirements.txt`), `skills/` has no
  `ci-troubleshooter/` directory, and the Vite app runs from the repository root
  rather than from `frontend/`. The entry point is `backend/main.py`
  (`app = FastAPI(...)`), not `backend/app/main.py`.

  Replaced with an accurate README: overview of the real components, the actual
  directory tree, quick start for the backend (`uvicorn main:app`) and the root
  Vite app (`npm run dev`), Supabase setup with backend/frontend key separation,
  both real env templates with their actual keys, and the real npm scripts.
